...
function SecurityView() {
  ...
  const confirmEnroll = async () => {
    await run(async () => {
      const r = await adminApi.mfaEnable(code.trim());
  ...
  const disable = async () => {
    ...
    const ok = await run(async () => { await adminApi.mfaDisable(disableCode.trim()); await load(); setStep("idle"); setDisableCode(""); }, { success: t("disabledToast") });
    if (ok) setEnabled(false);
  };

  if (enabled === null) return <div className="flex items-center gap-2 text-navy-500"><Loader2 className="h-5 w-5 animate-spin" /> {tc("loading")}</div>;
    ...
    <div className="max-w-2xl">
      ...
      <div className="rounded-xl border border-navy-100 bg-white p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${enabled ? "bg-emerald-50 text-emerald-700" : "bg-navy-100 text-navy-600"}`}>{enabled ? t("on") : t("off")}</span>
        </div>
          ...
          <div className="mt-5 space-y-4 border-t border-navy-100 pt-4">
            ...
            <div>
              ...
              <div className="mt-2 flex gap-2">
                <input autoFocus inputMode="numeric" value={code} onChange={(e) => setCode(e.target.value)} placeholder="••••••" className="w-40 rounded-lg border border-navy-200 px-3 py-2 text-center text-lg tracking-widest focus:border-accent-500 focus:outline-none" />
        ...
        {step === "recovery" && (
          <div className="mt-5 space-y-3 border-t border-navy-100 pt-4">
            ...
            <div className="grid grid-cols-2 gap-2 rounded-lg bg-navy-50 p-3 font-mono text-sm text-navy-800">{recovery.map((c) => <span key={c}>{c}</span>)}</div>
            <div className="flex gap-2">
              <button onClick={() => { navigator.clipboard?.writeText(recovery.join("\n")); toast.success(tc("toast.copied")); }} className="inline-flex items-center gap-1.5 rounded-lg border border-navy-200 px-3 py-1.5 text-sm hover:bg-navy-50"><Copy className="h-4 w-4" /> {t("copyCodes")}</button>
        ...
        {enabled && step === "idle" && (
          <div className="mt-5 flex flex-wrap items-end gap-2 border-t border-navy-100 pt-4">
            <div>
              ...
              <input inputMode="numeric" value={disableCode} onChange={(e) => setDisableCode(e.target.value)} placeholder="••••••" className="mt-1 block w-36 rounded-lg border border-navy-200 px-3 py-2 text-center tracking-widest focus:border-accent-500 focus:outline-none" />
            </div>
  ...
  return (
    <div className="mt-2">
      ...
      <div className="mt-1 flex items-center gap-2">
        ...
        <button onClick={() => { navigator.clipboard?.writeText(copyValue); setCopied(true); toast.success(tc("toast.copied")); setTimeout(() => setCopied(false), 1500); }} className="rounded-lg border border-navy-200 p-2 text-navy-500 hover:bg-navy-50" aria-label={tc("copy")}>
